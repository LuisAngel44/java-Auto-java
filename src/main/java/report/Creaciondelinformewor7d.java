...
public class Creaciondelinformewor7d {
    ...
    public static void generarReporteMasivo(String rutaExcel, String dirImg, String rutaSalida) {
        try (FileInputStream fis = new FileInputStream(new File(rutaExcel));
            ...
            while (rowIterator.hasNext()) {
                ...
                // Formateamos el código local a 6 dígitos
                ...
                // --- MODIFICACIÓN AQUÍ ---
                // 1. Buscamos las imágenes ideales
                ...
                // 2. Buscamos la imagen de ERROR por si acaso (ERROR_SALIDA_codigolocal)
                String rutaImgError = buscarImagen(dirImg, "ERROR_SALIDA", codigoLocal);

                // 3. Lógica de reemplazo: Si no hay imagen normal Y existe la de error, usamos la de error
                if (rutaImgDescarga == null && rutaImgError != null) {
                    rutaImgDescarga = rutaImgError;
                }
                
                if (rutaImgSalida == null && rutaImgError != null) {
                    rutaImgSalida = rutaImgError;
                }
                // -------------------------
    ...
    private static String buscarImagen(String carpeta, String tipo, String codigo) {
        ...
        if (files != null) {
            ...
            for (File f : files) {
                ...
                // Verifica si el nombre del archivo contiene el tipo
                if (name.contains(tipo)) {
                    // Verifica si contiene el código
