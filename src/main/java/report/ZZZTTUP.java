...
public class ZZZTTUP {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(">>> 🚀 INICIANDO ACTUALIZACIÓN MASIVA (Fecha Final siempre que no esté vacía) <<<");
        ...
        try (FileInputStream file = new FileInputStream(new File("iT1m4.xlsx"));
            ...
            for (Row row : sheet) {
                ...
                if (enviarPutV2(issueKey, putPayload, "Actualización General de Campos")) {
                    ...
                    // 3️⃣ POST-ACTUALIZACIÓN: ✅ FECHA FINAL SIEMPRE que exista (independiente de SOLUCIÓN)
                    String fechaSolRaw = getValRobust(row, COL_FECHA_SOL_IDX, formatter).trim();

                    if (!fechaSolRaw.isEmpty() && !isBorrar(fechaSolRaw)) {
                        System.out.println("   🗓️ Actualizando Fecha Final (customfield_10322)...");
                        ...
                        if (fSolFormateada != null) {
                            ...
                            System.out.println("   ⚠️ No se pudo formatear Fecha Final: '" + fechaSolRaw + "'. Se omite.");
                        }

                        String tSol = getValRobust(row, COL_TIEMPO_SOLUCION, formatter).trim();
                        if (!tSol.isEmpty() && !isBorrar(tSol)) {
                            fFields.put(FIELD_TIEMPO_SOLUCION, tSol);
                        }

                        if (fFields.size() > 0) {
                            enviarPutV2(issueKey, finalPayload, "Post-Resolución (Fecha Final + Tiempo Solución)");
                        }
                    }

                    // 4️⃣ (Opcional) Cambio de estado: si hay Fecha Final
                    if (!fechaSolRaw.isEmpty() && !isBorrar(fechaSolRaw)) {
    ...
    private static void prepararCamposActualizacion(ObjectNode fields, Row row, DataFormatter fmt, String issueKey) {
        ...
        // Fecha generación (se mantiene igual)
    ...
    private static java.io.File crearImagenConsolaEvidenciaSoloFechaFinal(String cid, java.time.LocalDateTime fechaSol, String carpetaSalida) throws Exception {
        new java.io.File(carpetaSalida).mkdirs();

        java.time.format.DateTimeFormatter fmtJuniper = java.time.format.DateTimeFormatter.ofPattern("MMM dd HH:mm:ss", java.util.Locale.ENGLISH);
        ...
        java.time.format.DateTimeFormatter fmtNombre = java.time.format.DateTimeFormatter.ofPattern("dd-MM-yyyy_HH-mm-ss");
        String nombre = "EVIDENCIA_" + cid + "_" + fechaSol.format(fmtNombre) + ".png";

        java.io.File out = new java.io.File(carpetaSalida, nombre);
        ...
        for (DateTimeFormatter f : fmts) {
            try {
                if (val.length() == 10 && val.contains("/")) {
                    return LocalDateTime.parse(val + " 00:00:00", DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));
                }
                if (val.length() == 10 && val.contains("-")) {
                    return LocalDateTime.parse(val + " 00:00:00", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
                }
